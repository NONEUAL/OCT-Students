...
import SchoolInfoModal from "./SchoolInfoModal.jsx";
  ...
  return (
    <>
      <aside className={`sidebar ${collapsed ? "sidebar--collapsed" : ""}`}>
        ...
        {/* School Info Button */}
        <div className={`sidebar__info-wrap ${collapsed ? "sidebar__info-wrap--collapsed" : ""}`}>
          <button
            className={`sidebar__info-btn ${collapsed ? "sidebar__info-btn--collapsed" : ""}`}
            onClick={() => setShowInfo(true)}
            title={collapsed ? "School Information" : undefined}
          >
            <Info size={15} strokeWidth={1.8} />
            {!collapsed && <span>School Information</span>}
          </button>
        </div>
      ...
      {showInfo && <SchoolInfoModal onClose={() => setShowInfo(false)} />}
    </>
  );
}
